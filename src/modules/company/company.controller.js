...
export const exportCompanyDb = async (req, res) => {
  ...
  try {
    ...
    const dumps = [
      ...
      ["customer_contacts", `customer_id IN (SELECT customer_id FROM ${env.dbPrefix}customer WHERE company_id = ${companyId})`],
      ["tickets", `company_id = ${companyId}`],
      ["ticket_history", `ticket_id IN (SELECT ticket_id FROM ${env.dbPrefix}tickets WHERE company_id = ${companyId})`],
      ["tickets_comments", `ticket_id IN (SELECT ticket_id FROM ${env.dbPrefix}tickets WHERE company_id = ${companyId})`],
